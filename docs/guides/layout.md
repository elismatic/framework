...
This guide will teach you the basics of layout, i.e. the sizing and positioning of elements in 3D space. All layout is controlled within a components's behaviors and is directly affected by it's location in the tree. 
...
Differential size can be used in addition to or in place of proprtional size. Differential size is simply a pixel value added to the size of the parent.
...
We recommend using the `'scale'` behavior for animating changes in size. Unlike the `'size'` behavior, scale is hardware accelerated and responds a lot better to changes, especially in DOM instensive applications. Note that `'origin'` affects how a node is scaled (see below).
...
<span class="sidenote"><strong>Still don't understand?</strong> Take a Post-it note and place it flat (sticky side up) on your desk. Now, take a single finger and use it to move the Post-it to the other side of your desk. Think of your finger as the mount point. Not setting the mount-point would be the equivalent of placing your figer at the top left of the Post-it before moving it. Setting mount point to [0.5,0.5] would be the equivalent of placing your finger in the middle of the Post-it and moving it along.</span>   
