...
var DATA = [{ 'name': 'Analog Clock', 'selector': 'clock', 'icon': 'clock-icon.jpg' }, { 'name': 'Weather', 'selector': 'weather', 'icon': 'weather-icon.jpg' }, { 'name': 'Calendar', 'selector': 'calendar', 'icon': 'calendar-icon.jpg' }, { 'name': 'Stocks', 'selector': 'stocks', 'icon': 'stocks-icon.jpg' }];
    ...
    events: {
        ...
        '.container': {
            ...
            'click': function ($state, $timelines) {
                ...
                if ($state.get('widgetListActive')) {
                    ...
                    $timelines.get('hide-available-widgets').start({ duration: 500 }, callback);
                }

                if ($state.get('removing')) {
                    $state.set('removing', false);
                    $timelines.get('hide-remove-buttons').start({ duration: 300 });
                }
            }
        },
        ...
        '.add': {
            'click': function ($state, $timelines, $event) {
                ...
                if (widgetListActive) {
                    $timelines.get('show-available-widgets').start({ duration: 500 }, callback);
                } else {
                    $timelines.get('hide-available-widgets').start({ duration: 500 }, callback);
                }
        ...
        '.remove': {
            ...
            'click': function ($state, $event, $timelines) {
                ...
                if (removing) {
                    $timelines.get('show-remove-buttons').start({ duration: 300 });
                } else {
                    $timelines.get('hide-remove-buttons').start({ duration: 300 });
                }
                ...
                if ($state.get('widgetListActive')) {
                    $state.set('widgetListActive', false);
                    $timelines.get('hide-available-widgets').start({ duration: 500 });
                }
            }
        },
        ...
        '.remove-dashboard-item': {
            ...
            'click': function ($event, $state, $timelines, $dispatcher) {
                ...
                $timelines.get('hide-' + widget).start({ duration: 600 });

                if (!$state.get('clockActive') && !$state.get('calendarActive') && !$state.get('weatherActive') && !$state.get('stocksActive')) {
        ...
        '.widget-list-item': {
            ...
            'click': function ($state, $payload, $index, $repeatPayload, $timelines) {
                ...
                $timelines.get('show-' + $repeatPayload.model.selector).start({ duration: 1000, delay: 300 });
            }
        }
    },
